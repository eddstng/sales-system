<template>
  <div>
    <v-dialog
      persistent
      @keydown.esc="selectedCustomerDetails.createCustomerFormDialog = false"
    ...
    updateCustomerSubmit: async function (selectedCustomer) {
      try {
        ...
        const selectedCustomerId =
          selectedCustomer.id ?? this.$store.state.selectedCustomer.id;
        if (isNaN(selectedCustomerId)) {
          throw new Error(
            "Error occured while trying to update customer details due to invalid customer id."
          );
        }
        ...
        if (res.status !== 200) {
          throw new Error(
            "Error occured while trying to update customer details during PUT request."
          );
        }
    ...
    checkIfIsDeliveryAndHasValidAddress: function () {
      if (
        ...
        (this.selectedCustomerDetails.selectedCustomer.street_number == "" ||
          ...
          this.selectedCustomerDetails.selectedCustomer.street_number == null ||
